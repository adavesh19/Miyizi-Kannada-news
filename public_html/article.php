...
$sourceName    = (string) ($article['source'] ?? 'Source');

$related = array_filter(
    articles_for_category($catSlug, 10),
    static fn(array $item): bool => ($item['slug'] ?? '') !== ($article['slug'] ?? '')
);
$relatedArr  = array_values($related);
$relatedTop  = array_slice($relatedArr, 0, 5);
$relatedMore = array_slice($relatedArr, 5, 3);
...
if (is_array($keyPoints) && count($keyPoints) >= 2) {
    $faqItems = [];
    foreach (array_slice($keyPoints, 0, 5) as $i => $pt) {
        $faqItems[] = [
            '@type' => 'Question',
            'name'  => 'Point ' . ($i + 1) . ': ' . mb_substr((string) $pt, 0, 100),
...
<div class="container art-page">
    ...
    <nav class="cat-breadcrumb" aria-label="breadcrumb">
        ...
        <a href="<?= e(category_url($catSlug)) ?>"><?= e($catLabel) ?></a><span aria-hidden="true">›</span>
    ...
    <div class="art-layout">
        ...
        <article class="art-body" itemscope itemtype="https://schema.org/NewsArticle">
            ...
            <!-- Category + time + source + live badge -->
            <div class="art-meta">
                ...
                <a class="art-meta__src" href="<?= e($sourceUrl) ?>" target="_blank" rel="nofollow noopener"><?= e($sourceName) ?> ↗</a>
                <span class="art-meta__badge">● LIVE</span>
            </div>
            ...
            <figure class="art-figure" itemprop="image" itemscope itemtype="https://schema.org/ImageObject">
                <div class="art-figure__wrap">
                    <img src="<?= e(article_image($article)) ?>"
                         alt="<?= e(excerpt_text((string) ($article['title'] ?? ''), 100)) ?>"
                         loading="eager"
                         onerror="this.onerror=null;this.src='<?= e(MIYIZE_FALLBACK_IMAGE) ?>';"
                         itemprop="url">
                    <div class="art-figure__shine"></div>
                    <div class="art-figure__edge-glow"></div>
                </div>
            </figure>

            <!-- Compact share strip (top) -->
            <div class="art-share-strip">
                <span class="art-share-label">Share:</span>
                <a class="art-share-btn art-share-btn--wa" href="https://wa.me/?text=<?= e(rawurlencode($shareText . ' ' . $articleUrlAbs)) ?>" target="_blank" rel="noopener noreferrer" title="WhatsApp">
                ...
                <a class="art-share-btn art-share-btn--x" href="https://twitter.com/intent/tweet?text=<?= e(rawurlencode($shareText)) ?>&url=<?= e(rawurlencode($articleUrlAbs)) ?>" target="_blank" rel="noopener noreferrer" title="X / Twitter">
                ...
                <a class="art-share-btn art-share-btn--tg" href="https://t.me/share/url?url=<?= e(rawurlencode($articleUrlAbs)) ?>&text=<?= e(rawurlencode($shareText)) ?>" target="_blank" rel="noopener noreferrer" title="Telegram">
                ...
                <a class="art-share-btn art-share-btn--fb" href="https://www.facebook.com/sharer/sharer.php?u=<?= e(rawurlencode($articleUrlAbs)) ?>" target="_blank" rel="noopener noreferrer" title="Facebook">
                    <svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <button class="art-share-btn art-share-btn--copy" id="copyBtn" onclick="navigator.clipboard.writeText('<?= e(addslashes($articleUrlAbs)) ?>');document.getElementById('copyBtn').classList.add('copied');setTimeout(()=>document.getElementById('copyBtn').classList.remove('copied'),2000)" title="Copy link">
                    <svg viewBox="0 0 24 24" fill="currentColor"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>
                </button>
            </div>

            <!-- Article paragraphs with mid-article ad injection -->
            ...
            <!-- ── KEY HIGHLIGHTS (premium box) ─────────────────────────── -->
            <?php if (!empty($keyPoints) && is_array($keyPoints)): ?>
            <section class="art-highlights" aria-label="ಮುಖ್ಯಾಂಶಗಳು">
                <div class="art-highlights__head">
                    <div class="art-highlights__icon-wrap">
                    ...
                    <h2>ಪ್ರಮುಖ ಮುಖ್ಯಾಂಶಗಳು</h2>
                    <span class="art-highlights__tag">Main Highlights</span>
                </div>
                <ul class="art-highlights__list">
                    <?php foreach ($keyPoints as $point): ?>
                        <li>
                            <span class="art-hl-dot"></span>
                            <span><?= e((string) $point) ?></span>
                        </li>
                    <?php endforeach; ?>
                </ul>
                <div class="art-highlights__source">
                    ...
                    <span>ಮೂಲ:</span>
                    <a href="<?= e($sourceUrl) ?>" target="_blank" rel="nofollow noopener"><?= e($sourceName) ?></a>
                    <span class="art-highlights__time"><?= e(format_kn_date((string) ($article['published_at'] ?? ''))) ?></span>
                </div>
            </section>
            <?php endif; ?>

            <!-- Video hub -->
            <section class="art-social-hub" aria-label="ವಿಡಿಯೋ">
                <h2>📺 ಸಂಬಂಧಿತ ವಿಡಿಯೋ</h2>
                <div class="art-social-btns">
                    ...
                    <a class="share-btn share-btn--x"         href="https://twitter.com/intent/tweet?text=<?= e(rawurlencode($shareText)) ?>&url=<?= e(rawurlencode($articleUrlAbs)) ?>" target="_blank" rel="noopener noreferrer">𝕏 X Post</a>
                ...
                <div class="video-embed">
                    <iframe src="<?= e($youtubeEmbed) ?>" title="YouTube Kannada News"
            ...
            <div class="art-source-box">
                <h2>📰 ಮೂಲ ವರದಿ</h2>
                <p>ಈ ಸುದ್ದಿಯ ಮೂಲ: <strong><?= e($sourceName) ?></strong>. ಸಂಪೂರ್ಣ ವರದಿಯನ್ನು ಓದಲು ಮೂಲ ಲಿಂಕ್ ಕ್ಲಿಕ್ ಮಾಡಿ.</p>
                <a class="button" href="<?= e($sourceUrl) ?>" target="_blank" rel="nofollow noopener">ಮೂಲ ಸುದ್ದಿ ಓದಿ →</a>
            </div>

            <!-- Bottom share repeat -->
            <div class="art-share-strip art-share-strip--bottom">
                <span class="art-share-label">Share this story:</span>
                <a class="art-share-btn art-share-btn--wa"  href="https://wa.me/?text=<?= e(rawurlencode($shareText . ' ' . $articleUrlAbs)) ?>" target="_blank" rel="noopener" title="WhatsApp"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></a>
                <a class="art-share-btn art-share-btn--x"   href="https://twitter.com/intent/tweet?text=<?= e(rawurlencode($shareText)) ?>&url=<?= e(rawurlencode($articleUrlAbs)) ?>" target="_blank" rel="noopener" title="X"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.259 5.631zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>
                <a class="art-share-btn art-share-btn--tg"  href="https://t.me/share/url?url=<?= e(rawurlencode($articleUrlAbs)) ?>&text=<?= e(rawurlencode($shareText)) ?>" target="_blank" rel="noopener" title="Telegram"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg></a>
            </div>
        </article>
        ...
        <aside class="art-sidebar">
            ...
            <div class="art-sidebar__block">
                ...
                <?php foreach ($relatedTop as $item): ?>
            ...
            <div class="art-sidebar__newsletter">
                ...
                <h3>WhatsApp ಅಲರ್ಟ್</h3>
                <p>ತಾಜಾ ಕನ್ನಡ ಸುದ್ದಿ ನೇರ ನಿಮ್ಮ ಫೋನಿಗೆ – ಉಚಿತ!</p>
                <a class="button" href="/contact.php">Subscribe →</a>
            </div>

            <!-- More category articles -->
            <?php if (!empty($relatedMore)): ?>
            <div class="art-sidebar__block">
                <div class="art-sidebar__head">
                    <h2><?= e($catLabel) ?></h2>
                    <a href="<?= e(category_url($catSlug)) ?>">More →</a>
                </div>
                <?php foreach ($relatedMore as $item): ?>
                    <?php render_article_card($item, 'compact'); ?>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
